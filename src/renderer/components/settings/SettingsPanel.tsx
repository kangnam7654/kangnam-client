...
/* ── MCP Tab ───────────────────────────────────── */
function MCPTab({ servers, newServerJson, setNewServerJson, onAdd, onRemove }: {
    ...
    <div>
      ...
      {servers.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {servers.map(server => (
            <div key={server.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: 12, background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button
                ...
                style={{ padding: '6px 14px', borderRadius: 8, border: 'none', fontSize: 13, color: 'var(--danger)', background: 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}
