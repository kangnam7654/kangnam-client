...
// Session ID to prevent unauthorized follow-ups
static COWORK_SESSION_ID: std::sync::LazyLock<Mutex<Option<String>>> =
    std::sync::LazyLock::new(|| Mutex::new(None));
...
pub async fn cowork_start(
    ...
    *COWORK_SESSION_ID.lock().unwrap_or_else(|e| e.into_inner()) = Some(session_id);
...
async fn run_cowork_loop(
    ...
    loop {
        ...
        if result.stop_reason == StopReason::ToolUse && !result.tool_calls.is_empty() {
            ...
            // Continue to next iteration (was: recursive Box::pin call)
            continue;
        }
